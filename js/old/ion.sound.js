...
;(function (window, navigator, $, undefined) {
    ...
    if (typeof Audio !== "function" && typeof Audio !== "object") {
        ...
        ion.sound.fade = func;
        ion.sound.bend = func;
        ion.sound.speedTransition = func;
        func();
        return;
    }
    ...
    ion.sound = function (options) {
        ...
        settings.convolver = settings.convolver || false;
    ...
    /**
        @desc fades in or out a sound
        @args name = the sound to fade, options = {fade_start: start of fade, fade_end: duration, fade_target: value of 0 to 1}
     */
    ion.sound.fade = function (name, options) {
        ion.sound._method("fade", name, options);
    };

    ion.sound.bend = function (name, options) {
        ion.sound._method("bend", name, options);
    };

    ion.sound.speedTransition = function (name, options) {
        ion.sound._method("speedTransition", name, options);
    };

    ion.sound.convolver = function (name, options) {
        ion.sound._method("convolver", name, options);
    };

    ion.sound.getBuffer = function(name) {
        return sounds[name].getBuffer();
    };

    ion.sound.isAudioContext = function () {
        if(audio) {
            return true;
        }
        return false;
    ...
    // sound object is directly accessible 
    ...
    Sound.prototype = {
        ...
        createUrl: function () {
            ...
            this.url = this.options.path + name.replace(/%2F/g, "/") + "." + this.options.supported[this.ext] + "?" + no_cache;
        },
        ...
        setBuffer: function (buffer) {
            if (this.options.convolver) {
                this.options.sprite = null;
                this.autoplay = false;
            }
        ...
        fade: function (options) {
            ...
            this.streams[0].fade(this.options);
        },

        bend: function (options) {
            if (options) {
                extend(options, this.options);
            } else {
                return;
            ...
            this.streams[0].bend(this.options);
        },

        speedTransition: function (options) {
            if (options) {
                extend(options, this.options);
            } else {
                return;
            ...
            this.streams[0].speedTransition(this.options);
        },
        ...
        convolver: function (options) {
            ...
            this.streams[0].convolver(this.options);
        }
    };
    ...
    var Stream = function (options, sprite_part) {
        ...
        this.convolverBuffer = options.convolverBuffer;
    ...
    Stream.prototype = {
        ...
        play: function (options) {
            ...
            this.source.detune.value = this.pitch;
        ...
        fade: function(options) {
            if (this.gain) {
                console.log(options.fade_target);
                console.log(options.fade_start);
                console.log(options.fade_end);
                this.gain.gain.setTargetAtTime(options.fade_target, audio.currentTime + options.fade_start, options.fade_end);
            }
        },

        bend: function(options) {
            if (this.source) {
                this.source.detune.setTargetAtTime(options.bend_target, audio.currentTime + options.bend_start, options.bend_end);
            }
        },

        speedTransition: function(options) {
            if (this.source) {
                this.source.playbackRate.setTargetAtTime(options.speed_target, audio.currentTime + options.speed_start, options.speed_end);
            }
        }
    };
    ...
    Stream.prototype = {
        ...
        createUrl: function () {
            ...
            this.url = encodeURI(this.path + name.replace(/%2F/g, "/") + "." + settings.supported[0] + "?" + rand);
        },
